...
class Preprocesamiento():
    ...
    def __init__(self,path_data):
        ...
        self.df = pd.DataFrame(columns=['file_path', 'mfccs_40', 'chroma', 'mel', 'contrast', 'tonnetz', 'duration', 'label'])
        self.df = self.df.fillna(0)  # with 0s rather than NaNs
        print('iniciando preprocesamiento de audios con caracteristicas almacenadas en {}'.format(path_data))

    def plot_specgram(self, file_path):
    ...
    def process(self, file, label):
        try:
            ...
            row = pd.Series({'file_path': file, 'mfccs_40': mfccs_40, 'chroma': chroma, 'mel': mel, 'contrast': contrast,
